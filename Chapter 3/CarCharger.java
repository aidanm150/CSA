public class CarCharger {
    ...
    public int getChargingCost(int startHour, int numHours){
        int cost = 0;
        int numHoursA = numHours;
        for(int i = 0;i<rateTable.length;i++){
            if(i>=startHour&&i<=startHour+(numHours-1)){
                cost+=rateTable[i];
                numHoursA--;
            }
        }
        if(numHoursA>0){
            while(numHoursA>0){
                for(int i = 0;i<rateTable.length&&numHoursA>0;i++){
                    cost+=rateTable[i];
                    numHoursA--;
                }
            }
    ...
    public int getChargeStartTime(int hours){
        int costSaved = getChargingCost(0,hours);
        int hourBest = 0;
        for(int i = 1; i<rateTable.length;i++){
            int k = getChargingCost(i, hours);
            if(k<costSaved){
                costSaved = k;
                hourBest = i;
            }
